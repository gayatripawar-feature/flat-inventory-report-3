...
//           }
//         }
//  console.log("Hold expired, updating backend:", f.flat_key, "hold_until:", f.hold_until);
//         return f;
...
//     setFlats(data);
//     setFlatStatus(statusMap);
//     // console.log("Flat after processing:", f.flat_key, f.status, f.hold_until);
...
//     if (formData.status === "hold") {
//       const hold_until = new Date(Date.now() + 10 * 1000); // +10 seconds
...
//       const parts = new Intl.DateTimeFormat("sv-SE", options).formatToParts(hold_until);
//       const dateStr = `${parts.find(p => p.type === "year").value}-${parts.find(p => p.type === "month").value}-${parts.find(p => p.type === "day").value}`;
//       const timeStr = `${parts.find(p => p.type === "hour").value}:${parts.find(p => p.type === "minute").value}:${parts.find(p => p.type === "second").value}`;

//       payload.hold_until = `${dateStr} ${timeStr}`; // ✅ snake_case
//     } else {
...
//     //     status: formData.status,
//     //     date: formData.date,
//     //     hold_until: payload.hold_until, // ✅ consistent
//     //   },
//     // }));
//     console.log("Updated flatStatus state:", {
//   [selectedFlat.flatKey]: {
//     status: formData.status,
//     date: formData.date,
...
//           ? "Flat put on hold for 10 seconds!"
//           : result.message,
//       severity: formData.status === "hold" ? "info" : "success",
//     });
//   } catch (err) {
...
//         //   const holdUntil = new Date(f.hold_until);
//         //   if (holdUntil < now) {
//         //     // Update backend when hold expired
//         //     await fetch(`/api/flats/${f.flat_key}`, {
//         //       method: "PUT",
//         //       headers: { "Content-Type": "application/json" },
//         //       body: JSON.stringify({
...
//         //       }),
//         //     });
//         //     f.status = "unsold"; // update locally too
//         //     f.hold_until = null;
//         //   }
//         // }
//         if (f.status === "hold" && f.hold_until) {

//   const holdUntil = new Date(f.hold_until.replace(" ", "T"));
//   console.log("Checking flat:", f.flat_key, "hold_until:", holdUntil.toISOString(), "now:", now.toISOString());
...
//       data.map(async (f) => {
//         if (f.status === "hold" && f.hold_until) {
//           // Parse hold_until safely as local datetime
//           const holdUntil = new Date(f.hold_until.replace(" ", "T"));
//           console.log(
//             "Checking flat:", f.flat_key,
//             "hold_until (parsed):", holdUntil,
//             "now:", now
//           );
...
//     // Build flatStatus map for quick access in UI
//     const statusMap = {};
//     data.forEach((f) => {
//       statusMap[f.flat_key] = {
//         status: f.status,
...
//           const holdUntil = new Date(f.hold_until + "Z"); // Z forces UTC
//           console.log(
//             "Checking flat:", f.flat_key,
...
//   // Save in local DB-friendly format (YYYY-MM-DD HH:MM:SS)
//   const localHoldUntil =
//     holdUntil.getFullYear() + "-" +
//     String(holdUntil.getMonth() + 1).padStart(2, "0") + "-" +
//     String(holdUntil.getDate()).padStart(2, "0") + " " +
//     String(holdUntil.getHours()).padStart(2, "0") + ":" +
//     String(holdUntil.getMinutes()).padStart(2, "0") + ":" +
//     String(holdUntil.getSeconds()).padStart(2, "0");

//   payload.hold_until = localHoldUntil;
//   }
